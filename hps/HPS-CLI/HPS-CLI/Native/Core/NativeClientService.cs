...
public sealed class NativeClientService
{
    ...
    private long _localEncryptionNonceCounter;
    ...
    public void AddKnownServer(string server)
    {
        ...
        {
            Console.Error.WriteLine($"[WARN] Invalid server address rejected: {server}");
            return;
        }
        if (string.IsNullOrWhiteSpace(parsed.Host))
        {
            Console.Error.WriteLine($"[WARN] Server address has no host: {server}");
            return;
        }
    ...
    private void SaveVoucherInternal(VoucherRecord voucher, bool persistState)
    {
        ...
        if (!string.Equals(path, legacyPath, StringComparison.OrdinalIgnoreCase) && File.Exists(legacyPath))
        {
            ...
            {
                Console.Error.WriteLine($"[WARN] SaveVoucherInternal delete legacy path failed: {ex.Message}");
            }
        }
    ...
    public string BuildContractTemplate(string actionType, IReadOnlyDictionary<string, string> details)
    {
        ...
        var merged = new Dictionary<string, string>(details, StringComparer.OrdinalIgnoreCase);
        if (!merged.ContainsKey("NONCE"))
            merged["NONCE"] = Convert.ToBase64String(System.Security.Cryptography.RandomNumberGenerator.GetBytes(16));
        if (!merged.ContainsKey("TIMESTAMP"))
            merged["TIMESTAMP"] = DateTimeOffset.UtcNow.ToUnixTimeSeconds().ToString();
    ...
    public bool VerifyContractSignatureWithKey(string contractText, string publicKeyPemOrBase64)
    {
        ...
        {
            Console.Error.WriteLine($"[WARN] VerifyContractSignatureWithKey failed: {ex.Message}");
            return false;
        }
    }
        ...
        if (count > 0)
        {
            foreach (var file in Directory.EnumerateFiles(_ctx.Paths.VouchersDir, "*.hps", SearchOption.AllDirectories))
            {
                if (!expectedFiles.Contains(file))
                {
                    ...
                    {
                        Console.Error.WriteLine($"[WARN] SaveWalletSync delete stale file failed: {ex.Message}");
                    }
                }
            }
            Save();
    ...
    public Dictionary<string, object?>? ParseHpsVoucherHsyst(string text)
    {
        ...
        Dictionary<string, object?> ParseJsonObj(string raw)
        {
            ...
            {
                Console.Error.WriteLine($"[WARN] ParseJsonObj failed: {ex.Message}");
    ...
    private DkvhpsVoucherInfo? BuildDkvhpsVoucherInfo(VoucherRecord voucher)
    {
        ...
        {
            Console.Error.WriteLine($"[WARN] BuildDkvhpsVoucherInfo failed: {ex.Message}");
            return null;
        }
    }
    ...
    private string BuildVoucherStoragePath(VoucherRecord voucher)
    {
        ...
        {
            Console.Error.WriteLine($"[WARN] BuildVoucherStoragePath parse failed: {ex.Message}");
        }
    ...
    private byte[] ProtectVoucherWithDkvhps(VoucherRecord voucher, byte[] plain)
    {
        ...
        {
            Console.Error.WriteLine($"[WARN] ProtectVoucherWithDkvhps failed: {ex.Message}");
            return plain;
        }
    }
    ...
    private byte[] EncryptForLocalStorage(byte[] plain)
    {
        ...
        try
        {
            ...
            var counter = Interlocked.Increment(ref _localEncryptionNonceCounter);
            System.Buffers.Binary.BinaryPrimitives.WriteInt64BigEndian(nonce.AsSpan(4), counter);
            RandomNumberGenerator.Fill(nonce.AsSpan(0, 4));
            var cipher = new byte[plain.Length];
            var tag = new byte[AesTagSize];
            try
            {
    ...
    private byte[] DecryptFromLocalStorage(byte[] input)
    {
        ...
        {
            Console.Error.WriteLine("[WARN] DecryptFromLocalStorage: file too small to be encrypted; returning as-is");
            return input;
        }
        for (var i = 0; i < EncMagic.Length; i++)
        {
            ...
            {
                Console.Error.WriteLine("[WARN] DecryptFromLocalStorage: file has no encryption header; returning as-is");
                return input;
            }
        }
    ...
    private void EncryptLegacyFilesInDirectory(string directory, string pattern)
    {
        ...
        foreach (var file in Directory.EnumerateFiles(directory, pattern, SearchOption.TopDirectoryOnly))
        {
            ...
            {
                Console.Error.WriteLine($"[WARN] EncryptLegacyFilesInDirectory failed for {file}: {ex.Message}");
            }
        }
    }
    ...
    private void Save()
    {
        ...
        Console.Error.WriteLine("[WARN] Saving state without encryption (crypto not unlocked)");
