...
public sealed partial class NativeCommandShell
{
    ...
    private async Task<int> RunCommandCoreAsync(string[] args, CancellationToken ct)
    {
        ...
        switch (cmd)
        {
            ...
            case "cancel":
                return await RunCancel(args.Skip(1).ToArray(), ct).ConfigureAwait(false);
            case "fraud-report":
                return await RunFraudReport(args.Skip(1).ToArray(), ct).ConfigureAwait(false);
            case "fee":
                return await RunFee(args.Skip(1).ToArray(), ct).ConfigureAwait(false);
            case "sync":
    ...
    private int RunKeys(string[] args)
    {
        ...
        switch (action)
        {
            ...
            case "show":
            {
                ...
                {
                    Console.Error.WriteLine($"[WARN] keys show Base64 decode failed: {ex.Message}");
            ...
            case "export":
            {
                ...
                var confirm = _display.GetInput("Tem certeza que deseja exportar a chave privada? (s/N)");
                if (!string.Equals(confirm, "s", StringComparison.OrdinalIgnoreCase))
                {
                    _display.PrintError("exportacao cancelada");
                    return 1;
                }
                try
                {
            ...
            case "export-bundle":
            {
                ...
                // Sanitize path to prevent path traversal
                path = Path.GetFullPath(path);
                var pass = _display.GetInput("Senha da chave para exportar bundle: ", password: true);
                if (string.IsNullOrWhiteSpace(pass))
                {
                    _display.PrintError("senha obrigatoria para exportar bundle");
                    return 3;
                }
                try
                {
                    _ctx.KeyManager.ExportEncryptedKeyBundle(user, path, pass);
            ...
            default:
                _display.PrintError("uso: keys [status [username]|init <username>|unlock <username>|lock|show|export-public|export <file_path>|import <file_path>|export-bundle <username> <output_path>|import-bundle <username> <input_path>]");
                return 2;
        }
    }
    ...
    private async Task<int> RunDownload(string[] args, CancellationToken ct)
    {
        ...
        {
            _display.PrintError("caminho de saida invalido (fora do diretorio atual)");
            return 2;
        }
    ...
    private async Task<int> RunLogin(string[] args, CancellationToken ct)
    {
        if (args.Length < 2)
        {
            _display.PrintError("uso: login <server> <username>");
            return 2;
        }
        ...
        {
            var passphrase = Environment.GetEnvironmentVariable("HPS_KEY_PASSPHRASE");
            ...
            {
                _display.PrintError("falha ao desbloquear cofre: verifique a senha");
                return 3;
            }
        }
    ...
    private string FormatCommandError(Exception ex)
    {
        ...
        {
            return $"falha de conexao com servidor {server}";
        }
        var msg = ex.Message;
        ...
        {
            return "operacao falhou (verifique os dados informados)";
        }
    ...
    private async Task<int> RunVoucher(string[] args, CancellationToken ct)
    {
        ...
        if (action == "audit")
        {
            ...
            {
                Console.Error.WriteLine($"[WARN] voucher audit parse failed: {ex.Message}");
            }
            return 0;
        }
        if (action == "contract")
    ...
    private async Task<int> RunExchange(string[] args, CancellationToken ct)
    {
        ...
        if (action == "validate")
        {
            ...
            {
                Console.Error.WriteLine($"[WARN] exchange validate token parse failed: {ex.Message}");
            }
            return 0;
        }
        if (action == "confirm")
        ...
        _display.PrintError("uso: exchange validate <target_server> <id1,id2,...> | exchange confirm [token_json_file]");
        return 2;
    }
    ...
    private int RunDkvhps(string[] args)
    {
        ...
        if (args[0].Equals("show", StringComparison.OrdinalIgnoreCase))
        {
            ...
            foreach (var voucher in lineage.Vouchers)
            {
                ...
                Console.WriteLine($"  voucher_key=****");
                Console.WriteLine($"  lineage_key=****");
    ...
    private void PrintHelp()
    {
        ...
        Console.WriteLine("cancel content <hash>");
        Console.WriteLine("cancel dns <domain>");
        Console.WriteLine("cancel flow <kind> <target>");
        ...
        Console.WriteLine("fraud-report <server_address> <reason>");
        ...
        Console.WriteLine("fee quotes");
        Console.WriteLine("fee market");
        Console.WriteLine("fee config [--min N] [--max N] [--volatility N] [--enable|--disable]");
    ...
    private void PersistContractsFromJson(string rawJson)
    {
        ...
        {
            Console.Error.WriteLine($"[WARN] PersistContractsFromJson failed: {ex.Message}");
        }
    }
    ...
    private static string DecodeContractContent(string value)
    {
        ...
        {
            Console.Error.WriteLine($"[WARN] DecodeContractContent failed: {ex.Message}");
            return value;
        }
    }
