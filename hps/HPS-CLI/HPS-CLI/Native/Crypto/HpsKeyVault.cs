...
public sealed class HpsKeyVault
{
    ...
    private (RSA loginPrivateKey, string loginPublicKeyPem, string localPublicKeyPem) LoadExistingKeysFromPaths(string username, string passphrase, string masterPath, string loginPath, string localPath)
    {
        var normalized = NormalizeUsername(username);
        ValidateInputs(normalized, passphrase);

        var masterKey = DecryptMasterKeyFromPath(normalized, passphrase, masterPath);
        try
        {
            var loginEnvelope = JsonSerializer.Deserialize<EncryptedKeyEnvelope>(File.ReadAllText(loginPath, Encoding.UTF8))
                ?? throw new InvalidOperationException("Arquivo de chave de login inválido.");
            var localEnvelope = JsonSerializer.Deserialize<EncryptedKeyEnvelope>(File.ReadAllText(localPath, Encoding.UTF8))
                ?? throw new InvalidOperationException("Arquivo de chave local inválido.");
    ...
    public void ExportEncryptedKeyBundle(string username, string outputPath, string passphrase)
    {
        var normalized = NormalizeUsername(username);
        ...
        // Verify the passphrase is correct before exporting
        try
        {
            var (_, _, _) = LoadExistingKeys(normalized, passphrase);
        }
        catch
        {
            throw new InvalidOperationException("Senha da chave inválida para exportação.");
        }
    ...
    public void ImportEncryptedKeyBundle(string username, string inputPath, string passphrase)
    {
        ...
        // Write to temporary files first, then atomically rename to prevent data loss on interruption
        var tempMaster = masterPath + ".tmp";
        var tempLogin = loginPath + ".tmp";
        var tempLocal = localPath + ".tmp";
        try
        {
            File.WriteAllBytes(tempMaster, Convert.FromBase64String(bundle.MasterKeyFile));
            File.WriteAllBytes(tempLogin, Convert.FromBase64String(bundle.LoginKeyFile));
            File.WriteAllBytes(tempLocal, Convert.FromBase64String(bundle.LocalKeyFile));

            // Validate the new keys before committing
            var (loginKey, _, _) = LoadExistingKeysFromPaths(normalized, passphrase, tempMaster, tempLogin, tempLocal);
            loginKey.Dispose();
            ...
            File.Move(tempMaster, masterPath, overwrite: true);
            File.Move(tempLogin, loginPath, overwrite: true);
            File.Move(tempLocal, localPath, overwrite: true);
        }
        catch
        {
            ...
            // Clean up temp files
            try { if (File.Exists(tempMaster)) File.Delete(tempMaster); }
            catch (Exception ex) { Console.Error.WriteLine($"[WARN] cleanup tempMaster failed: {ex.Message}"); }
            try { if (File.Exists(tempLogin)) File.Delete(tempLogin); }
            catch (Exception ex) { Console.Error.WriteLine($"[WARN] cleanup tempLogin failed: {ex.Message}"); }
            try { if (File.Exists(tempLocal)) File.Delete(tempLocal); }
            catch (Exception ex) { Console.Error.WriteLine($"[WARN] cleanup tempLocal failed: {ex.Message}"); }
            throw;
        }
        finally
        {
    ...
    private byte[] DecryptMasterKeyFromPath(string username, string passphrase, string masterKeyPath)
    {
        var envelope = JsonSerializer.Deserialize<MasterKeyEnvelope>(File.ReadAllText(masterKeyPath, Encoding.UTF8))
            ?? throw new InvalidOperationException("Arquivo de chave mestre inválido.");
