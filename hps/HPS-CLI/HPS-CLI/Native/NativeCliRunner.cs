...
public sealed class NativeCliRunner
{
    ...
    private async Task<int> RunPipeControllerAsync(NativeCommandShell shell, string pipeFilePath, CancellationToken ct)
    {
        ...
        _pipeToken = Convert.ToHexString(tokenBytes).ToLowerInvariant();
        var tokenFilePath = pipeFilePath + ".token";
        await File.WriteAllTextAsync(tokenFilePath, _pipeToken, Encoding.UTF8, ct).ConfigureAwait(false);
        try
        {
            if (OperatingSystem.IsWindows())
            {
                var fi = new FileInfo(tokenFilePath);
        ...
        {
            // Non-critical: continue if permission setting fails
        }
        ...
        while (!ct.IsCancellationRequested)
        {
            ...
            var command = ExtractPipeCommand(raw, requireToken: true);
    ...
    private string ExtractPipeCommand(string raw, bool requireToken = false)
    {
        ...
        if (requireToken)
        {
            ...
            if (_pipeToken is null || !string.Equals(lines[0], _pipeToken, StringComparison.Ordinal))
            {
                return string.Empty;
            }
            var cmdLine = lines[1];
            if (cmdLine.StartsWith("cmd:", StringComparison.OrdinalIgnoreCase))
            {
                return cmdLine[4..].Trim();
            }
            return cmdLine.Trim();
        }
