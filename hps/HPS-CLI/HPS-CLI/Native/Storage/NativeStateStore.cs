...
public sealed class NativeStateStore
{
    ...
    private long _encryptionNonceCounter;
    ...
    {
        _paths = paths;
        // Initialize nonce counter with random value to prevent predictability
        var randomBytes = new byte[8];
        RandomNumberGenerator.Fill(randomBytes);
        _encryptionNonceCounter = BitConverter.ToInt64(randomBytes, 0);
    }
    ...
    public NativeState Load()
    {
        ...
        {
            Console.Error.WriteLine("[WARN] state file not found; returning empty state");
        ...
        {
            Console.Error.WriteLine("[WARN] state file appears encrypted but no storage key provided; returning empty state");
            return new NativeState();
        }
    ...
    public void SaveEncrypted(NativeState state, byte[] storageKey)
    {
        _paths.EnsureDirectories();
        var raw = JsonSerializer.Serialize(state, JsonOptions);
        var enc = EncryptState(raw, storageKey, ref _encryptionNonceCounter);
    ...
    private static byte[] EncryptState(string json, byte[] key, ref long nonceCounter)
    {
