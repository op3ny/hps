...
public sealed class HpsSocketClient : IAsyncDisposable
{
    ...
    public async Task DisconnectAsync()
    {
        ...
        {
            Console.Error.WriteLine($"[WARN] HpsSocketClient.DisconnectAsync close failed: {ex.Message}");
        }
        finally
        {
            _connected = false;
    ...
    private async Task HandleIncomingAsync(string message, CancellationToken ct)
    {
        ...
        foreach (var callback in callbacks)
        {
            ...
            {
                Console.Error.WriteLine($"[WARN] HpsSocketClient handler callback failed: {ex.Message}");
            }
        }
    }
