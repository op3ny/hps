...
func (s *Server) runQueuedAction(conn socketio.Conn, action string, requestID string, fn func()) {
	...
	queueKind := queueActionToKind(action)
	emitFlowProgress(conn, queueKind, "queued", 1, 3, fmt.Sprintf("Adicionado à fila (%s)", action), 10000)
	...
	case <-time.After(60 * time.Second):
		log.Printf("WARN: action queue timeout for action=%s request_id=%s sid=%s", action, requestID, conn.ID())
	}

	if !ticketReady {
		s.releaseAction(ticket)
		emitFlowProgress(conn, queueKind, "timeout", 2, 3, "Tempo de fila excedido", 0)
		conn.Emit("action_queue_update", map[string]any{
			"action":     action,
			"request_id": requestID,
			"status":     "timeout",
	...
	emitFlowProgress(conn, queueKind, "processing", 2, 3, "Processando no servidor...", 30000)
	conn.Emit("action_queue_update", map[string]any{
		"action":     action,
		"request_id": requestID,
		"status":     "processing",
	...
	defer func() {
		s.releaseAction(ticket)
		emitFlowProgress(conn, queueKind, "done", 3, 3, "Operação concluída", 0)
		conn.Emit("action_queue_update", map[string]any{
			"action":     action,
			"request_id": requestID,
			"status":     "done",
...
func queueActionToKind(action string) string {
	switch action {
	case "authenticate":
		return "login"
