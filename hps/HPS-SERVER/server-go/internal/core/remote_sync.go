...
func (s *Server) ensureAddressProtocol(addr string) string {
...
func (s *Server) tryRequestWithFallback(ctx context.Context, client *http.Client, req *http.Request, serverAddress string, bodyBytes []byte) (*http.Response, string, error) {
	resp, err := client.Do(req)
	if err == nil {
		return resp, serverAddress, nil
	}
	errMsg := err.Error()
	isTLSError := strings.Contains(errMsg, "tls") || strings.Contains(errMsg, "certificate") || strings.Contains(errMsg, "handshake failure") || strings.Contains(errMsg, "http: server gave HTTP response to HTTPS client")
	isConnRefused := strings.Contains(errMsg, "connection refused") || strings.Contains(errMsg, "no route to host") || strings.Contains(errMsg, "connection reset by peer")
	if !isTLSError && !isConnRefused {
		return nil, serverAddress, err
	}
	parsed, parseErr := url.Parse(serverAddress)
	...
	altURL := strings.TrimRight(altAddr, "/") + req.URL.Path
	if req.URL.RawQuery != "" {
		altURL += "?" + req.URL.RawQuery
	}
	var altBody io.Reader
	if len(bodyBytes) > 0 {
		altBody = bytes.NewReader(bodyBytes)
	}
	altReq, altErr := http.NewRequestWithContext(ctx, req.Method, altURL, altBody)
	if altErr != nil {
		return nil, serverAddress, err
	}
...
func (s *Server) MakeRemoteRequestJSON(serverAddress, path, method string, data map[string]any, ctx ...context.Context) (bool, map[string]any, string) {
	...
	serverAddress = s.ensureAddressProtocol(serverAddress)
	if method == "" {
		method = http.MethodGet
	}
	urlStr := strings.TrimRight(serverAddress, "/") + path
	var body io.Reader
	var bodyBytes []byte
	if data != nil {
	...
	req, err := http.NewRequestWithContext(requestCtx, method, urlStr, body)
	if err != nil {
	...
	resp, usedAddr, err := s.tryRequestWithFallback(requestCtx, client, req, serverAddress, bodyBytes)
	if err != nil {
		return false, nil, err.Error()
	}
	...
	s.RememberKnownServer(usedAddr)
	return true, out, ""
}

func (s *Server) MakeRemoteRequestBytes(serverAddress, path, method string, ctx ...context.Context) (bool, []byte, string) {
	requestCtx := context.Background()
	if len(ctx) > 0 && ctx[0] != nil {
		requestCtx = ctx[0]
	}
	...
	req, err := http.NewRequestWithContext(requestCtx, method, strings.TrimRight(serverAddress, "/")+path, nil)
	if err != nil {
		return false, nil, err.Error()
	}
	...
	resp, usedAddr, err := s.tryRequestWithFallback(requestCtx, client, req, serverAddress, nil)
	if err != nil {
		return false, nil, err.Error()
	}
	defer resp.Body.Close()
...
func (s *Server) fetchServerID(addr string) (string, float64) {
	addr = s.ensureAddressProtocol(addr)
	start := time.Now()
	client := &http.Client{
	...
	resp, err := client.Get(strings.TrimRight(addr, "/") + "/server_info")
	if err != nil {
		errMsg := err.Error()
		isTLSError := strings.Contains(errMsg, "tls") || strings.Contains(errMsg, "certificate") || strings.Contains(errMsg, "handshake failure") || strings.Contains(errMsg, "http: server gave HTTP response to HTTPS client")
		isConnRefused := strings.Contains(errMsg, "connection refused") || strings.Contains(errMsg, "no route to host") || strings.Contains(errMsg, "connection reset by peer")
...
func (s *Server) SyncContentWithServer(serverAddress string) (int, error) {
	...
	for _, item := range items {
		...
		if _, statErr := os.Stat(filePath); statErr != nil {
			okRaw, contentBytes, _ := s.MakeRemoteRequestBytes(serverAddress, "/content/"+url.PathEscape(contentHash)+"?recursion_guard=1", http.MethodGet)
...
func (s *Server) FetchContentFromKnownServers(contentHash string) bool {
	...
	for _, serverAddr := range servers {
		...
		go func() {
			...
			okMeta, payload, _ := s.MakeRemoteRequestJSON(addr, "/sync/content?content_hash="+url.QueryEscape(contentHash), http.MethodGet, nil)
			items := []any{}
			if okMeta {
				items = castSlice(payload["items"])
			}
			...
			select {
			case results <- &knownServerResult{serverAddr: addr, raw: raw, items: items}:
			default:
			}
		}()
	}

	var firstResult *knownServerResult
	for i := 0; i < len(servers); i++ {
		select {
		case r := <-results:
			if r != nil && len(r.raw) > 0 && len(r.items) > 0 {
				firstResult = r
				cancel()
				goto processContent
			}
