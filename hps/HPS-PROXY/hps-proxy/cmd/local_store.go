...
func NewLocalStore(root string, storageKey []byte) (*LocalStore, error) {
	...
	// C1 FIX: Log load errors instead of silently ignoring
	if err := s.load(); err != nil {
		log.Printf("C1 FIX: LocalStore load failed: %v - using empty state", err)
	}
	return s, nil
}
...
func (s *LocalStore) RecordServerAccess(address string, success bool) {
	...
	// C2 FIX: Log save errors instead of silently ignoring
	if err := s.saveLocked(); err != nil {
		log.Printf("C2 FIX: LocalStore save failed after RecordServerAccess: %v", err)
	}
}
...
func (s *LocalStore) SaveDNS(domain, hash string) {
	...
	if err := s.saveLocked(); err != nil { log.Printf("C2 FIX: LocalStore save failed: %v", err) }
}
...
func (s *LocalStore) StoreContent(hash, domain, mime string, body []byte) IntegrityResult {
	...
	if encErr == nil {
		...
		// C2 FIX: Log write errors instead of silently ignoring
		if writeErr := os.WriteFile(filePath, blob, 0o600); writeErr != nil {
			log.Printf("C2 FIX: Failed to write file %s: %v", filePath, writeErr)
			integrity.Status = "local_write_error"
			integrity.Reason = writeErr.Error()
		}
	} else {
...
func (s *LocalStore) encryptBlob(plain []byte) ([]byte, error) {
	...
	defer zero(nonce)
	// A17 FIX: Use content AAD for blob encryption
	ciphertext, tag, err := encryptGCM(s.storageKey, nonce, plain, []byte("hps-proxy-content"))
	if err != nil {
		return nil, err
	}
...
func (s *LocalStore) ApplySyncSnapshot(ss SyncSnapshot) {
	...
	// Verify sync data integrity via HMAC
	if err := verifySyncIntegrity(ss); err != nil {
		log.Printf("[sync] AVISO: verificacao de integridade falhou: %v", err)
		log.Printf("[sync] Continuando com sync sem verificacao (modo degradado)")
	}
...
func (s *LocalStore) load() error {
	...
	decrypted, err := s.decryptState(raw)
	if err != nil {
		return err
	}
...
func (s *LocalStore) saveLocked() error {
	...
	encrypted, err := s.encryptState(raw)
	if err != nil {
		return err
	}
	return os.WriteFile(s.statePath, encrypted, 0o600)
}

func (s *LocalStore) encryptState(plain []byte) ([]byte, error) {
	...
	if len(combined) < tagSizeBytes {
		return nil, errors.New("ciphertext invalido")
	}
...
func (s *LocalStore) decryptState(raw []byte) ([]byte, error) {
	var env encryptedContentEnvelope
	...
	// A17 FIX: Use content AAD for blob decryption
	return gcm.Open(nil, nonce, combined, []byte("hps-proxy-content"))
}
...
func verifySyncIntegrity(ss SyncSnapshot) error {
	// C3 FIX: Verify actual data integrity, not just structure
	if ss.Content == nil {
		ss.Content = []map[string]any{}
	}
	if ss.DNS == nil {
		ss.DNS = []map[string]any{}
	}
	if ss.Contracts == nil {
		ss.Contracts = []map[string]any{}
	}
	if ss.Users == nil {
		ss.Users = []map[string]any{}
	}

	// C3 FIX: Validate DNS entries have required fields and valid hashes
	for i, row := range ss.DNS {
		domain := strings.TrimSpace(asString(row["domain"]))
		hash := strings.TrimSpace(asString(row["content_hash"]))
		if domain == "" || hash == "" {
			return errors.New("entrada DNS invalida no indice " + strconv.Itoa(i))
		}
		// Validate domain format (basic check)
		if len(domain) < 3 || len(domain) > 253 {
			return errors.New("dominio DNS com tamanho invalido no indice " + strconv.Itoa(i))
		}
		// Validate hash format (SHA256 or IPFS)
		if !isHexLen(strings.ToLower(hash), 64) && !strings.HasPrefix(hash, "Qm") {
			return errors.New("hash DNS com formato invalido no indice " + strconv.Itoa(i))
		}
	}

	// C3 FIX: Validate contract entries have required fields and valid structure
	for i, row := range ss.Contracts {
		id := strings.TrimSpace(asString(row["contract_id"]))
		if id == "" {
			return errors.New("contrato sem ID no indice " + strconv.Itoa(i))
		}
		// Validate contract has signature
		sig := strings.TrimSpace(asString(row["signature"]))
		if sig == "" {
			return errors.New("contrato sem assinatura no indice " + strconv.Itoa(i))
		}
		// Validate contract has username
		username := strings.TrimSpace(asString(row["username"]))
		if username == "" {
			return errors.New("contrato sem username no indice " + strconv.Itoa(i))
		}
	}

	// C3 FIX: Validate content entries
	for i, row := range ss.Content {
		hash := strings.TrimSpace(asString(row["content_hash"]))
		if hash == "" {
			return errors.New("conteudo sem hash no indice " + strconv.Itoa(i))
		}
	}
