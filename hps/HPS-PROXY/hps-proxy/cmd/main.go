...
func main() {
	...
	powNonce := RunBootstrapPoW(cfg.User, cfg.Pass)
	log.Printf("[auth] bootstrap concluido (nonce=%d)", powNonce)
	_ = powNonce // stored for potential future use in server auth
	...
	serverAuthToken := generateAuthToken()
	api.SetAuthToken(serverAuthToken)
	proxyAuthToken := generateAuthToken()
	log.Printf("[auth] token de autenticacao remoto gerado (salve para uso no servidor)")
	log.Printf("[auth] token de autenticacao proxy local gerado")
	// SECURITY FIX: Don't log auth token in clear text
	log.Printf("[auth] PROXY AUTH TOKEN: [SAVED - use --show-token to display]")
	...
	handler := NewProxyHandler(api, store, cfg.Server, proxyAuthToken)
