...
public static class SessionState
{
    ...
    // C5 FIX: Thread-safe accessor that returns a copy of the key reference
    public static RSA? GetPrivateKey()
    {
        lock (_lock) { return _privateKey; }
    }
    
    // C5 FIX: Setter disposes old key and zeros memory before setting new one
    public static void SetPrivateKey(RSA? value)
    {
        lock (_lock)
        {
            ...
            {
                // C5 FIX: Zero the private key material before dispose
                try
                {
