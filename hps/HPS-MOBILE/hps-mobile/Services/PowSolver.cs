...
public sealed class PowSolver
{
        ...
        Action<PowProgress>? progressCallback = null)
    {
        ...
        Task RunWorker(int workerId)
        {
            return Task.Run(() =>
            {
                // Use random starting nonce to prevent predictable nonce sequences
                var nonceBytes = new byte[8];
                RandomNumberGenerator.Fill(nonceBytes);
                var randomStart = BitConverter.ToUInt64(nonceBytes, 0);
                var nonce = randomStart + (ulong)workerId;
